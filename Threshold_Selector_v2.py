...
#place to save the outputted plots (make sure to end in slash)
SAVE_PLOTS_DIR = "C:/Users/Reid.Yesson/Documents/testingRxTx/Results/"
...
plt.subplots_adjust(left = 0.3, top=1)
...
def run1(event):
    ...
    name = transmitter_filename.split("/")[-1][:-7] + "_dBm.png"
    axDir = SAVE_PLOTS_DIR + name
    fig.savefig(axDir, bbox_inches = extent.expanded(1.2, 1.2))
    print(df_comp_rolling.range)
    print("\n")
    print(df_comp_rolling.comp)
    print("\n")
    ...
    name = transmitter_filename.split("/")[-1][:-7] + "_Comp.png"
    ax2Dir = SAVE_PLOTS_DIR + name
    fig.savefig(ax2Dir, bbox_inches = extent2.expanded(1.2, 1.2))
    name = transmitter_filename.split("/")[-1][:-7] + "_Range.png"
    ax3Dir = SAVE_PLOTS_DIR + name
    extent3 = ax3.get_window_extent().transformed(fig.dpi_scale_trans.inverted())
    fig.savefig(ax3Dir, bbox_inches = extent3.expanded(1.2, 1.2))
    #ax3.plot(df.index,df.range,marker='.')
...
close1 = plt.axes([0.06, 0.20, 0.15, 0.03])
